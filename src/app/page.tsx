    ...
    <div className="flex flex-col h-screen bg-white text-gray-900 font-sans selection:bg-blue-100">
      <header className="bg-white border-b p-3 md:p-4 flex items-center justify-between sticky top-0 z-10 gap-2">
        ...
        <div className="flex flex-col items-start sm:items-center min-w-0 flex-1 px-2 relative h-full">
          <div className={`flex flex-col items-start sm:items-center transition-all duration-500 transform ${status === 'playing' || status === 'congrats' || status === 'failed' ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
            ...
            <div className="flex items-center justify-start sm:justify-center max-w-full relative">
              <span className="text-lg md:text-xl font-bold truncate leading-none" dangerouslySetInnerHTML={{ __html: targetWord }} />
              {targetWord && (
            ...
            {status === 'countdown' && (
              <div className="h-full flex flex-col items-center justify-center text-center px-8">
                <div className="mb-8 transition-all duration-500">
                  <span className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-2">{t.target}</span>
                  <h3 className="text-3xl md:text-5xl font-bold text-blue-600" dangerouslySetInnerHTML={{ __html: targetWord }} />
                </div>
                <div className="text-8xl md:text-9xl font-serif italic text-gray-900 animate-pulse w-full">{countdown}</div>
                <p className="text-xs md:text-sm text-gray-400 uppercase tracking-widest font-bold mt-8">{t.preparing}</p>
              </div>
            )}
